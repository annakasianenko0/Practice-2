...
def get_word_score(word, n):
    ...
    for char in word.lower():
        if char !='*':
            komp_1 += SCRABBLE_LETTER_VALUES[char]
            
    komp_2 = 7 * word_length - 3 * (n - word_length)

    return komp_1 * (komp_2 if komp_2 > 1 else 1)
...
def update_hand(hand, word):
    ...
    for char in word.lower():
        if char in res.keys():
            res[char] -= 1 if res[char] > 0 else 0
    return res
...
def is_valid_word(word, hand, word_list):
    ...
    for char in word_lower:
        if char not in hand_copy.keys() or hand_copy[char] == 0:
            return False
        hand_copy[char] -= 1
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while True:
        ...
        if is_valid_word(input_word, hand, word_list):
            ...
            print(f"\"{input_word}\" earned {earned_score} points. Total: {total_score} points")
        else:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while current_hand_num < hands_number:
        current_hand = deal_hand(HAND_SIZE)
        print("Current Hand: ", end="")
        display_hand(current_hand)
        if replays_left > 0:
            subs_hand = input("Would you like to substitute a letter? ")
            if subs_hand.lower() == 'yes':
        ...
        earned_score = play_hand(current_hand, word_list)
        if replays_left > 0:
...
if __name__ == '__main__':
    ...
    print(f"Total score over all hands: {play_game(word_list)}")
